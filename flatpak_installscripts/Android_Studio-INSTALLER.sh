...
aliascheck_=$(cat ~/.bash_aliases|grep -o -m 1 Android_Studio|head -1)
[ -z "$autoload_" ] && echo "$_bash_aliases" >> ~/.bashrc
[ -z "$aliascheck_" ] && echo "alias Android_Studio='flatpak run com.google.AndroidStudio'" >> ~/.bash_aliases 
echo "DONE."
}
_init
