...
echo "CREATING ALIAS FOR Cave_Story_NX"
[ ! -f ~/.bashrc ] && touch ~/.bashrc
_bash_aliases="[ -f ~/.bash_aliases ] && . ~/.bash_aliases"
autoload_=$(cat ~/.bashrc|grep "$_bash_aliases")
aliascheck_=$(cat ~/.bash_aliases|grep -o -m 1 Cave_Story_NX|head -1)
[ -z "$autoload_" ] && echo "$_bash_aliases" >> ~/.bashrc
[ -z "$aliascheck_" ] && echo "alias Cave_Story_NX='flatpak run com.gitlab.coringao.cavestory-nx'" >> ~/.bash_aliases 
echo "DONE."
}
_init
