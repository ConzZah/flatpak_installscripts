...
aliascheck_=$(cat ~/.bash_aliases|grep -o -m 1 Heroic_Games_Launcher|head -1)
[ -z "$autoload_" ] && echo "$_bash_aliases" >> ~/.bashrc
[ -z "$aliascheck_" ] && echo "alias Heroic_Games_Launcher='flatpak run com.heroicgameslauncher.hgl'" >> ~/.bash_aliases 
echo "DONE."
}
_init
