...
wget -q -O MakeMKV_icon.png https://dl.flathub.org/repo/appstream/x86_64/icons/128x128/com.makemkv.MakeMKV.png
$_doso mv MakeMKV_icon.png /etc/
_sc="MakeMKV.desktop"
cd /home/$USER/Desktop
echo "[Desktop Entry]">$_sc
echo "Version=1.0">>$_sc
echo "Type=Application">>$_sc
echo "Name=MakeMKV">>$_sc
echo "Comment=MakeMKV">>$_sc
echo "Exec=flatpak run com.makemkv.MakeMKV">>$_sc
echo "Icon=/etc/MakeMKV_icon.png">>$_sc
echo "Path=">>$_sc
echo "Terminal=false">>$_sc
echo "StartupNotify=false">>$_sc
echo "CREATING ALIAS FOR MakeMKV"
[ ! -f ~/.bashrc ] && touch ~/.bashrc
_bash_aliases="[ -f ~/.bash_aliases ] && . ~/.bash_aliases"
autoload_=$(cat ~/.bashrc|grep "$_bash_aliases")
aliascheck_=$(cat ~/.bash_aliases|grep -o -m 1 MakeMKV|head -1)
[ -z "$autoload_" ] && echo "$_bash_aliases" >> ~/.bashrc
[ -z "$aliascheck_" ] && echo "alias MakeMKV='flatpak run com.makemkv.MakeMKV'" >> ~/.bash_aliases 
echo "DONE."
}
_init
