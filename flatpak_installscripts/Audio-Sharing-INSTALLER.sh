...
aliascheck_=$(cat ~/.bash_aliases|grep -o -m 1 Audio-Sharing|head -1)
[ -z "$autoload_" ] && echo "$_bash_aliases" >> ~/.bashrc
[ -z "$aliascheck_" ] && echo "alias Audio-Sharing='flatpak run de.haeckerfelix.AudioSharing'" >> ~/.bash_aliases 
echo "DONE."
}
_init
